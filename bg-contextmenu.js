...
(function () {
    ...
    function markMenuRegistrationCompleted() {
        ...
        var createProps = {
            ...
            targetUrlPatterns: ['about:blank#unresolvable'] // Unresolvable URL.
        };
        try {
    ...
    function checkContextMenuPref() {
        ...
        storageArea.get({
            ...
            if (items && items.showContextMenu) {
                show();
    ...
    function contextMenusOnClicked(info, tab) {
        ...
        getPlatformInfoAsync(function () {
            url = get_crx_url(url);
            openExplorerRelatedToTab(url, tab);
        });
    }
    function openExplorerRelatedToTab(url, tab) {
        var params = encodeQueryString({ crx: url });

        chrome.tabs.create({
            url: chrome.runtime.getURL('explorer.html') + '?' + params,
            active: true,
    ...
    function show() {
        chrome.contextMenus.create({
            id: MENU_ID_LINK,
            title: 'CodeLens: View linked extension source',
            contexts: ['link'],
        ...
        chrome.contextMenus.create({
            ...
            title: 'CodeLens: View linked extension source (latest approved)',
            contexts: ['link'],
            targetUrlPatterns: amo_match_patterns.concat(atn_match_patterns),
        });
        chrome.contextMenus.create({
            id: MENU_ID_PAGE,
            title: 'CodeLens: View extension source',
            contexts: ['all'],
            documentUrlPatterns: [
                ...cws_match_patterns,
                mea_match_pattern,
        ...
        chrome.contextMenus.create({
            ...
            title: 'CodeLens: View extension source (latest approved)',
